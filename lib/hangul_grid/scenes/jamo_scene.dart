...
/// Dot-grid paper background (Moleskine-style notebook page).
///
/// Uses [kPaper] rather than the cream the sibling `hangul` project paints,
/// so the canvas is continuous with the page behind it instead of sitting on
/// it as a differently-tinted rectangle.
class PaperLayer extends Layer {
  static const _paperColor = kPaper;
...
/// [JamoLayer._classifyStemAndTicks]). [left]/[right] go with a vertical
...
/// time as classifiers are built; [JamoLayer.recognizedSounds] is kept
...
///   [JamoLayer._classifyStemAndTicks]).
...
///   [JamoLayer._classifyComposition]).
...
///   composition path as the tense consonants ([JamoLayer._classifyComposition]).
...
class JamoLayer extends Layer {
  JamoLayer({this.onCommit});

  /// Called with the compatibility jamo once a drawn letter settles — see
  /// [_idleCommitSeconds]. The canvas wipes itself immediately afterwards,
  /// ready for the next letter.
  final void Function(String jamo)? onCommit;

  /// How long the pen must be still before the recognized letter is accepted
  /// and appended to the word.
  ///
  /// It has to be a delay rather than "commit as soon as something is
  /// recognized", because the classifiers are prefix-ambiguous by
  /// construction: ㅏ is a strict subset of ㅑ and of ㅐ, ㅗ of ㅘ, ㄱ of ㅋ and
  /// ㅈ, ㄴ of ㄷ of ㄹ of ㅌ. [_classifyBase] resolves that by running
  /// most-strokes-first, but that only disambiguates a *finished* drawing.
  /// Committing on first recognition would append ㅏ while the hand is still
  /// on its way to drawing ㅑ.
  ///
  /// This is the same rule every handwriting IME uses. [commitNow] is the
  /// manual override for anyone who does not want to wait.
  static const double _idleCommitSeconds = 0.9;

  /// Seconds since the pen last did anything.
  double _idle = 0;
  ...
  /// [JamoRow.sound]. Kept in step with [_HangulJamo] — used by the legend
  /// to mute letters that can't be drawn yet rather than listing them as if
  /// they could.
  ...
  /// How far along the settle countdown the pending letter is, 0 to 1. Drawn
  /// as a filling bar under the readout so the commit is never a surprise.
  double get commitProgress =>
      (_idle / _idleCommitSeconds).clamp(0.0, 1.0).toDouble();

  /// Whether there is a recognized letter waiting to be accepted.
  bool get hasPending => _recognized != null && _activePoints == null;
  ...
  /// Accepts the pending letter without waiting out the countdown.
  void commitNow() {
    final recognized = _recognized;
    if (recognized == null || _activePoints != null) return;
    onCommit?.call(recognized.glyph);
    clear();
  }

  /// Runs the settle countdown off the canvas's existing ticker.
  ///
  /// Deliberately not a `dart:async` Timer: driving it from [update] keeps
  /// the layer free of async lifecycle, and lets the tests advance time by
  /// calling `update(0.1)` instead of pumping a fake clock.
  @override
  void update(double dt) {
    if (_recognized == null || _activePoints != null || _strokes.isEmpty) {
      return;
    }
    _idle += dt;
    if (_idle >= _idleCommitSeconds) commitNow();
  }

  @override
  void handlePointerEvent(PointerEvent event, Size size) {
    // Any pen activity restarts the countdown — including a pen-down that
    // has not lifted yet, so a slow multi-stroke letter is never cut off
    // partway through.
    _idle = 0;
  ...
  _HangulJamo? _classifyGiyeok() =>
      _isCorner(_strokes.last, bodyAboveChord: true)
          ? _HangulJamo.giyeok
          : null;
  ...
  _HangulJamo? _classifyWo() {
    ...
    final eoTick =
        identical(horizontals[0], bar) ? horizontals[1] : horizontals[0];
  ...
  _HangulJamo? _classifyWa() {
    ...
    if (_boundsOf(aTick.points).center.dx <= _boundsOf(stem.points).center.dx) {
      return null;
    }
  ...
  _HangulJamo? _classifyHieut() {
    ...
    bars.sort((x, y) =>
        _boundsOf(x.points).center.dy.compareTo(_boundsOf(y.points).center.dy));
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // The readout and its countdown are painted here rather than built as
    // widgets above the canvas: this layer already repaints every frame off
    // the ticker, so an animating bar costs nothing, whereas the widget
    // version would need a setState per frame.
    final recognized = _recognized;
    final label = TextPainter(
      text: recognized != null
          ...
          : TextSpan(
              text: 'Draw a letter here — it is added when you pause',
              style: TextStyle(
    ...
    final barHeight = recognized != null ? 3.0 + 8 : 0.0;
    final labelTop = size.height - 24 - label.height - barHeight;
    label.paint(canvas, Offset(24, labelTop));

    if (recognized != null) {
      const trackWidth = 96.0;
      final top = labelTop + label.height + 8;
      final track = Rect.fromLTWH(24, top, trackWidth, 3);
      canvas.drawRect(track, Paint()..color = kInk.withValues(alpha: 0.12));
      canvas.drawRect(
        Rect.fromLTWH(24, top, trackWidth * commitProgress, 3),
        Paint()..color = kPenColor,
      );
    }
  }
}

/// The canvas size every classifier tolerance in [JamoLayer] is tuned for.
///
/// The thresholds are absolute pixel distances — `_touchTolerance` of 22,
/// `_minCornerChord` of 15 — chosen for a letter drawn a couple of hundred
/// pixels tall. They are not ratios, so a caller writing into a smaller area
/// has to scale its points into a box this size before asking.
const double kRecognitionBox = 400;

/// Classifies an already-captured set of strokes as a single jamo, without
/// going through pointer events.
///
/// This is how the grid canvas recognizes a cell: it owns its own stroke
/// capture, and only needs the classifiers. Points must already be scaled
/// into a [kRecognitionBox]-sized box — see `grid_scene.dart`.
String? recognizeJamo(List<List<Offset>> strokes) {
  final layer = JamoLayer();
  for (final points in strokes) {
    if (points.length < 2) continue;
    layer._commit(_Stroke(points));
  }
  return layer.recognizedGlyph;
}

/// Builds the scene plus a direct reference to its [JamoLayer], so the
/// hosting page can clear it and force a commit from its buttons.
(Scene, JamoLayer) buildJamoScene({void Function(String jamo)? onCommit}) {
  final layer = JamoLayer(onCommit: onCommit);
