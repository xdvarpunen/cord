...
///
/// The same shim the Hanzi Grid page uses for its `?script=` — `flutter test`
/// runs on the VM, where importing `dart:html` is a *compile* error, so the one
/// web-only call goes behind a conditional import: the real thing on the web, a
/// no-op everywhere else. The Latin page is widget-tested, so it needs it.
///
