...
/// recognizer can't identify are muted (see [GreekLayer.recognizedNames]) so
...
/// drawable.
///
/// Shared by the desktop side panel on [GreekPage] and the full-screen
/// [ReferencePage], the same way the other script features share their table.
class GreekReference extends StatelessWidget {
  const GreekReference({required this.alphabet, super.key});
          ...
          Wrap(
            ...
            children: [
              for (final row in alphabet.rows)
                _LetterChip(
                  row,
                  disabled: !GreekLayer.recognizedNames.contains(row.name),
                ),
            ],
          ),
        ],
